...
static void ppu_getWindowState(Ppu* ppu, int layer);
...
void ppu_runLine(Ppu* ppu, int line) {
  if(line == 0) {
    ...
    for (int i = 0; i < 6; i++) {
      ppu->windowLayer[i].windowStateCacheUsed = false;
    }
    ppu_getWindowState(ppu, 5);
...
static int ppu_getPixel(Ppu* ppu, int x, int y, bool sub, int out[3]) {
  ...
  for(int i = 0; i < layerCountPerMode[actMode]; i++) {
    ...
    ppu_getWindowState(ppu, curLayer);
    bool windowStateForCurLayer = ppu->windowLayer[curLayer].windowStateCache[x];
    if(!sub) {
      layerActive = ppu->layer[curLayer].mainScreenEnabled && (
        !ppu->layer[curLayer].mainScreenWindowed || !windowStateForCurLayer
      );
    } else {
      layerActive = ppu->layer[curLayer].subScreenEnabled && (
        !ppu->layer[curLayer].subScreenWindowed || !windowStateForCurLayer
      );
    }
    if(layerActive) {
      if(curLayer < 4) {
...
static void ppu_getWindowState(Ppu* ppu, int layer) {
  ...
  if (wl->windowStateCacheUsed) {
    return;
  }

  bool e1 = wl->window1enabled;
  bool e2 = wl->window2enabled;

  bool* buf = wl->windowStateCache;

  if(!e1 && !e2) {
    memset(buf, 0, 256);
  } else if(e1 && !e2) {
    bool invert = wl->window1inversed;
    for (int x = 0; x < 256; x++) {
      buf[x] = (x >= ppu->window1left && x <= ppu->window1right) ^ invert;
    }
  } else if(!e1 && e2) {
    bool invert = wl->window2inversed;
    for (int x = 0; x < 256; x++) {
      buf[x] = (x >= ppu->window2left && x <= ppu->window2right) ^ invert;
    }
  } else {
    bool inv1 = wl->window1inversed;
    bool inv2 = wl->window2inversed;
    bool res1[256], res2[256];
    for (int x = 0; x < 256; x++) {
      res1[x] = (x >= ppu->window1left && x <= ppu->window1right) ^ inv1;
      res2[x] = (x >= ppu->window2left && x <= ppu->window2right) ^ inv2;
    }
    for (int x = 0; x < 256; x++) {
      switch(wl->maskLogic) {
        case 0: buf[x] = res1[x] || res2[x]; break;
        case 1: buf[x] = res1[x] && res2[x]; break;
        case 2: buf[x] = res1[x] != res2[x]; break;
        case 3: buf[x] = res1[x] == res2[x]; break;
      }
    }
  }

  wl->windowStateCacheUsed = true;
}
...
EMSCRIPTEN_KEEPALIVE bool* ppu_getOverscan(Ppu* p) { return &(p->overscan); }
EMSCRIPTEN_KEEPALIVE bool* ppu_getFrameOverscan(Ppu* p) { return &(p->frameOverscan); }
